use std::{
    error,
    fmt::{
        self,
        Debug,
        Display,
        Formatter
    },
    io,
    path::PathBuf
};

use crate::processors;

#[derive(Debug)]
pub enum Error {
    InvalidOutputPath(String),
    IO(io::Error),
    Load(LoadError),
    Save(SaveError),
    CantGenerateValidIdentifier,
    DirectoryExpected(PathBuf),
    InvalidVersion { version: String, expected: String }
}

impl error::Error for Error {
    fn source(&self) -> Option<&(dyn error::Error + 'static)> {
        match &self {
            Error::InvalidOutputPath(_) => None,
            Error::IO(io_error) => Some(io_error),
            Error::Load(load_error) => Some(load_error),
            Error::Save(save_error) => Some(save_error),
            Error::CantGenerateValidIdentifier => None,
            Error::DirectoryExpected(_path) => None,
            Error::InvalidVersion { .. } => None
        }
    }
}

impl Display for Error {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match &self {
            Error::InvalidOutputPath(path) => write!(f, "Invalid output path '{}'. Expected a valid directory.", path),
            Error::IO(io_error) => write!(f, "IO error: {}", io_error),
            Error::Load(load_error) => write!(f, "Error when loading a cache file: {}", load_error),
            Error::Save(save_error) => write!(f, "Error when saving a cache file: {}", save_error),
            Error::CantGenerateValidIdentifier => write!(f, "Can't generate a valid unused identifier after many tries."),
            Error::DirectoryExpected(path) => write!(f, "Directory expected at path '{}'", path.display()),
            Error::InvalidVersion { version, expected } => write!(f, "Invalid cache version '{}', expected version '{}'", version, expected)
        }
    }
}

impl Into<processors::Error> for Error {
    fn into(self) -> processors::Error {
        processors::Error::CacheProcessor(self)
    }
}



#[derive(Debug)]
pub enum LoadError {
    IO(io::Error),
    Deserialize(serde_json::error::Error)
}

impl error::Error for LoadError {
    fn source(&self) -> Option<&(dyn error::Error + 'static)> {
        match &self {
            LoadError::IO(io_error) => Some(io_error),
            LoadError::Deserialize(toml_error) => Some(toml_error)
        }
    }
}

impl Display for LoadError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "An error occured when trying to load a config file:")?;

        match &self {
            LoadError::IO(io_error) => write!(f, "IO error => {}", io_error),
            LoadError::Deserialize(toml_error) => write!(f, "Error when deserializing from a toml file => {}", toml_error)
        }
    }
}

impl Into<Error> for LoadError {
    fn into(self) -> Error {
        Error::Load(self)
    }
}



#[derive(Debug)]
pub enum SaveError {
    IO(io::Error),
    Serialize(serde_json::error::Error)
}

impl error::Error for SaveError {
    fn source(&self) -> Option<&(dyn error::Error + 'static)> {
        match &self {
            SaveError::IO(io_error) => Some(io_error),
            SaveError::Serialize(json_error) => Some(json_error)
        }
    }
}

impl Display for SaveError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "An error occured when trying to save a config file:")?;

        match &self {
            SaveError::IO(io_error) => write!(f, "IO error => {}", io_error),
            SaveError::Serialize(json_error) => write!(f, "Error when serializing into a json file => {}", json_error)
        }
    }
}

impl Into<Error> for SaveError {
    fn into(self) -> Error {
        Error::Save(self)
    }
}
